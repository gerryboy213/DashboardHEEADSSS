...
class User(db.Model):
    ...
    assigned_center = db.Column(db.String(100))  # current center responsible for user
...
class Notification(db.Model):
    ...
    center_name = db.Column(db.String(100))  # The receiving center (e.g., RHU)
    message = db.Column(db.Text)
    urgency = db.Column(db.String(10))  # Optional: "high", "medium", etc.
...
def admin_login():
    if request.method == 'POST':
        ...
        if admin_data and check_password_hash(admin_data.password, password):
            ...
            # ✅ Return plain response for Android
...
def admin_list():
    ...
    province_code, city_code = center_info
    admin_source = center_source_map.get(admin.center_name, "brgy")  # Default to brgy if unknown
    ...
    users_query = User.query.filter(
        ...
        or_(
            ...
            User.assigned_center == admin.center_name
        )
    )

    # Apply visibility filter based on center type
    if admin_source == "rhu":
        users_query = users_query.filter(User.visible_to_rhu == True)
    elif admin_source == "brgy":
        users_query = users_query.filter(User.visible_to_brgy == True)
    elif admin_source == "hospital":
        users_query = users_query.filter(User.visible_to_hospital == True)
...
def view_client(user_id):
    ...
    signature2 = Signature.query.filter_by(user_id=admin.id).order_by(Signature.id.desc()).first()
    ...
    last_center = user.referral_history.split(" → ")[-1] if user.referral_history else admin.center_name
    forwarding_chain = determine_forwarding_chain(last_center)

    try:
        current_index = forwarding_chain.index(admin.center_name)
    except ValueError:
        current_index = -1  # fallback if admin center not in chain
    ...
    # Add this logic before rendering the template
    forwarding_chain = determine_forwarding_chain(admin.center_name)
    try:
        current_index = forwarding_chain.index(admin.center_name)
        next_center = forwarding_chain[current_index + 1]
...
def save_services():
    try:
        ...
        source = session.get('source', 'unknown')  # fallback to 'unknown' if not set


        # Clear existing entries
        Services.query.filter_by(user_id=user_id, source=source).delete()
        Recommendations.query.filter_by(user_id=user_id, source=source).delete()
...
@app.route('/send_to_rhu', methods=['POST'])
def send_to_rhu():
    ...
    current_center = user.assigned_center or admin.center_name
    forwarding_chain = determine_forwarding_chain(current_center)

    if not forwarding_chain:
        return jsonify({'message': f'No forwarding path defined for {current_center}.'}), 400

    try:
        current_index = forwarding_chain.index(admin.center_name)
        next_center = forwarding_chain[current_index + 1]
    except (ValueError, IndexError):
        return jsonify({'message': 'No next center to forward to.'}), 400

    user.assigned_center = next_center
    user.last_forwarded_by = admin.full_name
    user.last_forwarded_by_position = admin.position
    ...
    # Keep brgy visibility if already set
    user.visible_to_brgy = user.visible_to_brgy or (center_source == "brgy")
    user.visible_to_rhu = 1 if center_source == "rhu" else user.visible_to_rhu
    user.visible_to_hospital = 1 if center_source == "hospital" else user.visible_to_hospital

    # ➕ Referral history
    if user.referral_history:
        user.referral_history += f" → {next_center}"
    else:
        user.referral_history = f"{admin.center_name} → {next_center}"

    # ➕ Notification
    notification = Notification(
        user_id=user.id,
        center_name=next_center,
        message=f"Client {user.control_num} forwarded to your center.",
        urgency="medium",
        unread=True
    )
    ...
    return jsonify({'message': f'User forwarded to {next_center} successfully!'}), 200
...
def delete_user(user_id):
    ...
    # Determine center
    center = admin.center_name.lower()

    # Update visibility flag
    if "rhu" in center:
        user.visible_to_rhu = False
    elif "brgy" in center or "bhs" in center:
        user.visible_to_brgy = False
    elif "dh" in center or "hospital" in center:
        user.visible_to_hospital = False
    else:
        return jsonify({"error": "Center type not recognized"}), 400
...
def get_notifications():
    ...
    all_answers = (
        ...
        .filter(func.lower(func.trim(User.location)) == func.lower(func.trim(admin.center_name)))
        .all()
    )
    ...
    # 🔧 FIXED: Forwarded notifications must be added **outside** of the loop above
    forwarded_notifications = Notification.query.filter_by(
        center_name=admin.center_name,
        unread=True
    ).all()

    for n in forwarded_notifications:
        user = User.query.get(n.user_id)
        if not user:
            continue

        user_answers = (
            db.session.query(UserResponse.question_number, UserResponse.response)
            .filter_by(user_id=user.id)
            .filter(UserResponse.question_number.in_(all_question_numbers))
            .all()
        )

        answers = {q: r for q, r in user_answers}
        if all(answers.get(qn) == "No" for qn in all_question_numbers):
            continue  # same logic: ignore if all are "No"
        ...
        notifications.append({
            "id": user.id,
            "user_id": user.id,
            "sender": user.control_num,
            "content": n.message,
            "submitted_at": n.timestamp,
            "urgency": urgency,
            "unread": n.unread
        })
